...
use modules::{
  ModuleManager, pig_game::PigGameModule, powerful_nahruk::PowerfulNahrukModule,
...
async fn handle_message(
      ...
      if let Some(command) = parts.first() {
        ...
        match *command {
          "help" => {
            ...
            bot
              .send_message(msg.chat.id, format!("Available commands:\n{}", help_text))
              .await?;
            return Ok(());
          }
          _ => {
  ...
  module_manager
    .handle_message(bot, msg, &db, &config)
    .await?;
  Ok(())
}
